"""
SAM.gov BAA Opportunity Monitor
Polls SAM.gov for new/updated Broad Agency Announcement notices and posts to Teams.
"""
...
def collect_all_opportunities(posted_from: str, posted_to: str) -> list[dict]:
    """Fetch all records in window, keep only BAA-titled ones, classify each."""
    raw = fetch_all_in_window(posted_from, posted_to)
    matches = []
    for opp in raw:
        if not is_baa_relevant(opp):
            continue
        opp["_matched_profile"] = classify_opportunity(opp)
        opp["_matched_emoji"]   = emoji_for_profile(opp["_matched_profile"])
        matches.append(opp)
    log.info("Relevance filter: %d BAA matches out of %d records", len(matches), len(raw))
    return matches


def is_baa_relevant(opp: dict) -> bool:
    title    = (opp.get("title") or "").upper()
    combined = f" {title} "
    return any(term in combined for term in BAA_TITLE_TERMS)


def classify_opportunity(opp: dict) -> str:
    title   = (opp.get("title") or "").upper()
    dept    = (opp.get("departmentName") or "").upper()
    subtier = (opp.get("subtierName") or "").upper()
    agency  = (opp.get("organizationHierarchy", [{}])[0].get("name", "") if opp.get("organizationHierarchy") else "").upper()
    all_text = f"{title} {dept} {agency} {subtier}"

    if any(x in dept for x in ["DEPARTMENT OF THE AIR FORCE", "DEPARTMENT OF THE ARMY", "DEPARTMENT OF THE NAVY",
                                "DEFENSE", "DOD"]):
        return "DoD BAA"
    if any(x in dept for x in ["NATIONAL SCIENCE FOUNDATION", "NSF"]):
        return "NSF BAA"
    if any(x in dept for x in ["NATIONAL INSTITUTES", "NIH", "NATIONAL INSTITUTE"]):
        return "NIH BAA"
    return "BAA"


def emoji_for_profile(label: str) -> str:
    mapping = {"DoD BAA": "🔵", "NSF BAA": "🟡", "NIH BAA": "🟣", "BAA": "🟢"}
...
def format_teams_message(opp: dict, is_update: bool) -> dict:
    ...
    label      = opp.get("_matched_profile", "BAA")
...
def post_run_summary(
    ...
    body = [
        _text("📊 SAM.gov BAA Monitor — Run Complete", weight="Bolder", size="Medium"),
